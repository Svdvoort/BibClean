...
def get_entry_type(type_info):
    entry_type_table = {'proceedings-article': 'inproceedings',
    ...
    for entry in bib_database.entries:
        ...
        if has_doi:
            if doi_tools.is_crossref_work(entry['doi']):
                ...
                entry = converter.json_to_bib(entry_info,
                                              entry,
                                              config)
                ...
                if doi_org_info is not None:
                    ...
                    warning_invalid_doi_string = "Entry {entry_id} has an invalid doi {doi}; removed DOI"
                    logging.warning(warning_invalid_doi_string.format(entry_id=entry["ID"],
                                                                      doi=entry['doi']))
                    entry.pop("doi", None)
                    has_doi = False
        ...
        entry['author'] = formatter.name_list_to_bib(entry['author'],
                                                     max_authors)


        if 'editor' in entry:
            entry['editor'] = formatter.name_list_to_bib(entry['editor'],
                                                         max_authors)
        ...
        if entry_hash not in hashes:
            ...
            if remove_duplicates:
                ...
                msg = inf_string.format(entry['ID'],
                                        hashes[entry_hash],
                                        entry['ID'])
                logger.warning(msg)
            else:
