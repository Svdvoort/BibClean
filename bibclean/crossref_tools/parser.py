...
def has_journal_issue_date(cr_info):
    ...
    if "journal-issue" in cr_info:
        ...
        for i_publication_type in publication_types:
            has_date = (
                i_publication_type in journal_info
                and "date-parts" in journal_info[i_publication_type]
            )
            if has_date:
                return has_date
    return False
...
def has_published_date(cr_info):
    publication_types = constants.PUBLICATION_TYPES
    for i_publication_type in publication_types:
        has_date = i_publication_type in cr_info and "date-parts" in cr_info[i_publication_type]
        if has_date:
            return has_date
    return False
...
def has_issued_date(cr_info):
    return "issued" in cr_info and "date-parts" in cr_info["issued"]


def has_publication_date(cr_info):
    has_date = (
        has_journal_issue_date(cr_info) or has_published_date(cr_info) or has_issued_date(cr_info)
    )
