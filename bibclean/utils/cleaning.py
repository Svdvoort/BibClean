...
def clean_unicode(text):
    ...
    for i_character in text:
        ...
        if len(encoded_character) > 1 and (
            encoded_character[0] != constants.L_BRACE or encoded_character[-1] != constants.R_BRACE
        ):
            encoded_character = encoded_character.replace(constants.R_BRACE, "").split(
                constants.L_BRACE
            )
...
def clean_text(text):
    ...
    for i_element in text_elements:
        ...
        if N_upper_case >= N_required and not contains_punctuation:
            ...
            N_upper_case >= N_required
            and i_element[0] in string.punctuation
            and i_element[-1] in string.punctuation
        ):
