...
def json_to_bib(json_entry, bib_entry, config):
    ...
    if config.get_update_pages():
        ...
        elif (parser.has_article_number(json_entry) and
              config.get_article_number_keyword() == 'pages'):
            bib_entry = cr_update.update_article_number(json_entry, bib_entry,
                                                        keyword='pages',
                                                        remove_pages=False)
...
def bib_to_bib(new_bib_entry, original_bib_entry, config):
    if config.get_update_authors():
        original_bib_entry = bib_update.update_authors(new_bib_entry,
                                                       original_bib_entry)
